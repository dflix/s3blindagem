...
class Email
{
    ...
    public function __construct()
    {
        ...
        $this->message = new \Source\Models\Message();
...
//        }
    ...
    public function sendQueue(int $perSecond = 5 ) {
        $stmt = \Source\Core\Connect::getInstance()->query("SELECT * FROM mail_queue WHERE sent_at IS NULL");
        if($stmt->rowCount()){
            foreach ($stmt->fetchAll() as $send) {
                ...
                if($email->send($send->from_email, $send->from_name)){
                    ...
                    \Source\Core\Connect::getInstance()->exec("UPDATE mail_queue SET sent_at = NOW() WHERE id = {$send->id}");
     ...
     * @return Message
     */
    public function message(): \Source\Models\Message
    {
        return $this->message;
    }
