...
function url(string $path = null): string {
    if (strpos($_SERVER['HTTP_HOST'], "www.localhost")) {
        if ($path) {
...
/**
 * Função para trazer a URL qualificada para inclusão de links no template views
 * @param string $theme
...
function cookie(): void {
    ...
    $filtro = filter_input_array(INPUT_COOKIE, FILTER_DEFAULT);

    $aff = filter_input_array(INPUT_GET, FILTER_SANITIZE_STRIPPED);

    if (!empty($filtro)) {

        if (!empty($aff["aff"])) {
            setcookie('dflix', "{$aff["aff"]}", (time() + (90 * 24 * 3600)));
        } else {
            if(!empty($_COOKIE["dflix"])){
            setcookie('dflix', "52", (time() + (90 * 24 * 3600)));
            }
        }
